        ...
        with urllib.request.urlopen(req, timeout=5, context=ssl_context) as response:
            if response.status == 200:
                ...
                if server_version > CURRENT_VERSION:
                    print(f"🚀 새 버전 발견 ({server_version})! 코드를 다운로드합니다.")
                    code_req = urllib.request.Request(UPDATE_CODE_URL, headers={"User-Agent": "Mozilla/5.0"})
                    with urllib.request.urlopen(code_req, timeout=10, context=ssl_context) as new_code_response:
                        if new_code_response.status == 200:
                            ...
                            with open(updated_file_path, "w", encoding="utf-8") as f:
                                f.write(new_code_response.read().decode("utf-8"))
...
# 2. 💡 [Bad magic number 완벽 예방] updated_main.py가 존재하면 동적 우회 실행
...
if os.path.exists(_updated_script) and __name__ == "__main__":
    try:
        ...
        with open(_updated_script, "r", encoding="utf-8") as _f:
            _code = _f.read()
        exec(compile(_code, _updated_script, 'exec'), {'__name__': '__main__', '__file__': _updated_script})
        sys.exit(0)
...
from kivy.properties import ListProperty, ObjectProperty, NumericProperty, DictProperty, StringProperty, BooleanProperty, ColorProperty
...
if platform == 'android':
    ...
    from jnius import PythonJavaClass, java_method, JavaException, autoclass
...
DEFAULT_FONT_STYLE = {'font_name': FONT_NAME, 'font_size': dp(15), 'color': TEXT_DARK}
...
class StyledButton(Button):
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            ...
            self.bg_rounded_rect = RoundedRectangle(pos=self.pos, size=self.size, radius=[dp(12),])
...
class StyledToggleButton(ToggleButton):
    def __init__(self, **kwargs):
        ...
        self.background_down = ''
        self.background_color = (0, 0, 0, 0)
        if 'font_size' not in kwargs: self.font_size = dp(14)
        with self.canvas.before:
    ...
    def _update_state(self, instance, value):
        if value == 'down':
            ...
            self.color = TEXT_DARK; self.bold = False
...
class StyledSpinner(Spinner):
    def __init__(self, **kwargs):
        ...
        if 'font_size' not in kwargs: self.font_size = dp(13)
        with self.canvas.before:
...
def execute_with_retry(func, *args, **kwargs):
    ...
    raise Exception("🚨 구글 API 트래픽 초과 오류 누적으로 작업이 최종 실패했습니다.")

def invalidate_cache(sheet_name):
    if sheet_name in g_cached_sheets: del g_cached_sheets[sheet_name]
    if sheet_name in g_cache_timestamps: del g_cache_timestamps[sheet_name]
...
def get_worksheet(worksheet_name):
    ...
    if not GSPREAD_LOADED:
        ...
        if not GSPREAD_LOADED or not g_spreadsheet:
            raise Exception(f"구글 키 파일('{SERVICE_ACCOUNT_FILE}')이 없거나 연결 실패했습니다.\n{GSPREAD_ERROR_MSG}")
...
def get_sheet_data(sheet_name, force_refresh=False):
    ...
    if not force_refresh and (now - last_updated) < CACHE_DURATION:
        if sheet_name in g_cached_sheets: return g_cached_sheets[sheet_name]
    try:
...
def t(d, k, default=''): return d.get(k, default)

# --- 팝업 모듈 ---
class LoadingPopup(Popup):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.title = '처리 중'; self.title_font = FONT_NAME
        self.content = Label(text='데이터를 처리하는 중입니다...\n잠시만 기다려주세요.', font_name=FONT_NAME, font_size=dp(16))
        self.size_hint = (0.8, 0.4); self.auto_dismiss = False

class InfoPopup(Popup):
    def __init__(self, title, message, **kwargs):
        super().__init__(**kwargs)
        self.title = title; self.title_font = FONT_NAME; self.size_hint = (0.9, 0.5)
        content = BoxLayout(orientation='vertical', padding=dp(10), spacing=dp(10))
        message_label = Label(text=str(message), font_name=FONT_NAME, font_size=dp(14), halign='center', valign='top')
        ...
        ok_button = StyledButton(text='확인', size_hint_y=None, height=dp(45)); ok_button.bind(on_press=self.dismiss)
        content.add_widget(ok_button)
        self.content = content

class SingleInputPopup(Popup):
    def __init__(self, title, hint_text, on_confirm, initial_text="", input_type='text', warning_text=None, **kwargs):
        super().__init__(**kwargs)
        self.title = title; self.title_font = FONT_NAME; self.size_hint = (0.9, None); self.auto_dismiss = False
        self.on_confirm = on_confirm
        
        main_layout = BoxLayout(orientation='vertical', spacing=dp(10), padding=dp(15), size_hint_y=None)
        main_layout.bind(minimum_height=main_layout.setter('height'))
        main_layout.bind(height=lambda instance, value: setattr(self, 'height', value + dp(60)))

        if warning_text:
            warning_label = Label(text=warning_text, font_name=FONT_NAME, font_size=dp(15), color=get_color_from_hex('#D32F2F'), markup=True, halign='center', size_hint_y=None)
            warning_label.bind(width=lambda *x: warning_label.setter('text_size')(warning_label, (warning_label.width, None)),
                               texture_size=lambda *x: warning_label.setter('height')(warning_label, warning_label.texture_size[1]))
            main_layout.add_widget(warning_label)
        
        self.text_input = TextInput(text=initial_text, hint_text=hint_text, multiline=False, size_hint_y=None, height=dp(50), font_size=dp(18), font_name=FONT_NAME, input_type=input_type)
        ...
        button_layout.add_widget(cancel_button); button_layout.add_widget(ok_button)
        main_layout.add_widget(self.text_input); main_layout.add_widget(button_layout)
...
class MultipleSkuSelectPopup(Popup):
    def __init__(self, matches, on_select, **kwargs):
        ...
        layout.add_widget(Label(text="스캔한 바코드가 여러 로케이션에 존재합니다.\n대상을 선택해주세요.", font_name=FONT_NAME, size_hint_y=None, height=dp(40)))
        ...
        for item in matches:
            ...
            qty = t(task, '지시수량', '0')
            btn = StyledButton(text=f"보관위치: {loc} | 지시수량: {qty}EA", size_hint_y=None, height=dp(55))
            btn.bind(on_release=lambda instance, i=item: (on_select(i), self.dismiss()))
            grid.add_widget(btn)
...
class InspectionPopup(Popup):
    def __init__(self, card, task_list_screen, **kwargs):
        ...
        if self.is_invoice_only:
            invoice_banner = Label(text="🏷️ [송장만 부착 대상] (입수량 1EA)", font_name=FONT_NAME, font_size=dp(15), bold=True, color=get_color_from_hex('#D32F2F'), size_hint_y=None, height=dp(30))
            main_layout.add_widget(invoice_banner)

        qty_grid = GridLayout(cols=2, spacing=dp(10), size_hint_y=None, height=dp(100))
        total_qty = safe_int(t(self.task_data, '지시수량', 0))
        confirmed_qty_str = str(t(self.task_data, '확인수량', ''))
        confirmed_qty_display = confirmed_qty_str if confirmed_qty_str.isdigit() and confirmed_qty_str else '미입력'

        qty_grid.add_widget(Label(text="총 지시수량:", font_name=FONT_NAME))
        qty_grid.add_widget(Label(text=str(total_qty), font_name=FONT_NAME, bold=True))

        qty_grid.add_widget(Label(text="보충 확인수량:", font_name=FONT_NAME))
        qty_grid.add_widget(Label(text=f"{total_qty} / [b]{confirmed_qty_display}[/b]", font_name=FONT_NAME, markup=True))

        main_layout.add_widget(qty_grid)
        
        hint_txt = "최종 적치위치 입력"
        self.final_location_input = TextInput(text=str(t(self.task_data, '보충로케이션', '')), hint_text=hint_txt, multiline=False, font_name=FONT_NAME, size_hint_y=None, height=dp(45))
        main_layout.add_widget(self.final_location_input)

        top_button_grid = GridLayout(cols=2, size_hint_y=None, height=dp(90), spacing=dp(10))
        cancel_button = StyledButton(text="취소", bg_color=(0.6, 0.6, 0.6, 1))
        cancel_button.bind(on_press=self.dismiss)
        ok_button = StyledButton(text="최종 검수 완료", bg_color=PRIMARY_BLUE)
    ...
    def confirm_inspection(self, instance):
        ...
        self.task_list_screen._finalize_task_processing(
            card=self.card,
            final_qty=safe_int(t(self.task_data, '확인수량', t(self.task_data, '지시수량', 0))),
            split_qty=0,
...
class NameEntryScreen(Screen):
    def __init__(self, **kwargs):
        ...
        layout.add_widget(Label(text='보충 업무 자동화', font_name=FONT_NAME, font_size=dp(28), bold=True, color=PRIMARY_BLUE, size_hint_y=0.3))

        input_box = BoxLayout(orientation='vertical', spacing=dp(10), size_hint_y=0.3)
        input_box.add_widget(Label(text='작업자 이름을 입력해주세요', font_name=FONT_NAME, font_size=dp(16), color=TEXT_DARK))
        self.name_input = TextInput(hint_text='예: 홍길동', multiline=False, font_name=FONT_NAME, font_size=dp(20), size_hint_y=None, height=dp(50), halign='center')
        input_box.add_widget(self.name_input)

        start_btn = StyledButton(text='작업 시작하기', size_hint_y=None, height=dp(55))
        start_btn.bind(on_press=self.start_app)
        
        layout.add_widget(input_box); layout.add_widget(start_btn); layout.add_widget(Widget(size_hint_y=0.2))
        self.add_widget(layout)

    def on_enter(self, *args):
        app = App.get_running_app()
        saved_name = app.load_saved_user_name()
        if saved_name: self.name_input.text = saved_name

    def start_app(self, instance):
        name = self.name_input.text.strip()
        if not name:
            App.get_running_app().show_info_popup("알림", "이름을 입력해야 합니다.")
            return
        app = App.get_running_app()
...
class MainMenuScreen(Screen):
    ...
    def on_enter(self, *args):
        ...
        welcome_box.add_widget(Label(text=f'"{app.user_real_name}"님', font_name=FONT_NAME, font_size=dp(18), bold=True, color=PRIMARY_BLUE, halign='left'))
        welcome_box.add_widget(Label(text='오늘도 안전 작업하세요!', font_name=FONT_NAME, font_size=dp(13), color=TEXT_MUTED, halign='left'))
        for child in welcome_box.children: child.bind(size=lambda i, s: setattr(i, 'text_size', s))

        btn_printer = StyledButton(text='프린터', size_hint_x=None, width=dp(70), font_size=dp(12), bg_color=get_color_from_hex('#78909C'))
        btn_printer.bind(on_press=lambda x: Factory.PrinterSettingsPopup().open())

        top_bar.add_widget(welcome_box); top_bar.add_widget(btn_printer)
        self.layout.add_widget(top_bar)
        
        dash_card = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(140), padding=dp(10), spacing=dp(4))
        with dash_card.canvas.before:
            Color(1, 1, 1, 1)
            RoundedRectangle(pos=dash_card.pos, size=dash_card.size, radius=[dp(12),])
        dash_card.bind(pos=lambda i,p: setattr(i.canvas.before.children[-1], 'pos', p), size=lambda i,s: setattr(i.canvas.before.children[-1], 'size', s))
        
        lbl_dash_title = Label(text='📊 실시간 보충 현황 요약', font_name=FONT_NAME, font_size=dp(13), bold=True, color=PRIMARY_BLUE, size_hint_y=None, height=dp(20), halign='left')
        ...
        grid.add_widget(Label(text='구분', font_name=FONT_NAME, font_size=dp(12), color=TEXT_MUTED, bold=True))
        grid.add_widget(Label(text='대기중 (긴급)', font_name=FONT_NAME, font_size=dp(12), color=TEXT_MUTED, bold=True))
        grid.add_widget(Label(text='작업중 (긴급)', font_name=FONT_NAME, font_size=dp(12), color=TEXT_MUTED, bold=True))

        self.lbl_op_pending = Label(text='0 [color=D32F2F](0)[/color]', font_name=FONT_NAME, font_size=dp(13), markup=True, color=TEXT_DARK)
        self.lbl_op_working = Label(text='0 [color=D32F2F](0)[/color]', font_name=FONT_NAME, font_size=dp(13), markup=True, color=PRIMARY_BLUE)
        grid.add_widget(Label(text='오더피커', font_name=FONT_NAME, font_size=dp(12), color=TEXT_DARK, bold=True))
        grid.add_widget(self.lbl_op_pending); grid.add_widget(self.lbl_op_working)

        self.lbl_reach_pending = Label(text='0 [color=D32F2F](0)[/color]', font_name=FONT_NAME, font_size=dp(13), markup=True, color=TEXT_DARK)
        self.lbl_reach_working = Label(text='0 [color=D32F2F](0)[/color]', font_name=FONT_NAME, font_size=dp(13), markup=True, color=PRIMARY_BLUE)
        grid.add_widget(Label(text='리치', font_name=FONT_NAME, font_size=dp(12), color=TEXT_DARK, bold=True))
        grid.add_widget(self.lbl_reach_pending); grid.add_widget(self.lbl_reach_working)
        ...
        sep.bind(pos=lambda i, p: setattr(rect, 'pos', p), size=lambda i, s: setattr(rect, 'size', s))
        dash_card.add_widget(sep)

        grid_tot = GridLayout(cols=3, size_hint_y=None, height=dp(25), spacing=dp(2))
        self.lbl_all_pending = Label(text='0 [color=D32F2F](0)[/color]', font_name=FONT_NAME, font_size=dp(13), bold=True, markup=True, color=TEXT_DARK)
        self.lbl_all_working = Label(text='0 [color=D32F2F](0)[/color]', font_name=FONT_NAME, font_size=dp(13), bold=True, markup=True, color=PRIMARY_BLUE)
        grid_tot.add_widget(Label(text='전체합계', font_name=FONT_NAME, font_size=dp(12), color=TEXT_DARK, bold=True))
        grid_tot.add_widget(self.lbl_all_pending); grid_tot.add_widget(self.lbl_all_working)
        ...
        def create_compact_menu_row(btn_widget):
            ...
            row.add_widget(Widget()); row.add_widget(btn_widget); row.add_widget(Widget())
            return row

        btn_replenish = StyledButton(text='[보충] 보충 작업', bg_color=PRIMARY_BLUE, size_hint_x=None, width=dp(220))
        btn_replenish.bind(on_press=lambda x: setattr(self.manager, 'current', 'unified_replenish'))
        menu_box.add_widget(create_compact_menu_row(btn_replenish))
            
        btn_inspect = StyledButton(text='[검수] 검수 목록 보기', bg_color=get_color_from_hex('#00897B'), size_hint_x=None, width=dp(220))
        btn_inspect.bind(on_press=self.go_to_inspect)
        menu_box.add_widget(create_compact_menu_row(btn_inspect))

        btn_dashboard = StyledButton(text='[현황] 전체 작업 현황판', bg_color=get_color_from_hex('#3F51B5'), size_hint_x=None, width=dp(220))
        btn_dashboard.bind(on_press=lambda x: setattr(self.manager, 'current', 'admin_dashboard'))
        menu_box.add_widget(create_compact_menu_row(btn_dashboard))
            
        self.layout.add_widget(menu_box); self.layout.add_widget(Widget())

        bottom_box = BoxLayout(size_hint_y=None, height=dp(36))
        bottom_box.add_widget(Widget())
        change_btn = StyledButton(text='작업자 이름 변경', size_hint_x=None, width=dp(140), bg_color=get_color_from_hex('#FF7043'), font_size=dp(12))
        change_btn.bind(on_press=lambda x: setattr(self.manager, 'current', 'name_entry'))
        bottom_box.add_widget(change_btn)
        self.layout.add_widget(bottom_box)
        
        threading.Thread(target=self._fetch_summary_counts, daemon=True).start()
    ...
    def _fetch_summary_counts(self):
        try:
            ...
            Clock.schedule_once(lambda dt: self._update_summary_labels(
                f"{op_p} [color=D32F2F]({op_pu})[/color]", f"{op_w} [color=D32F2F]({op_wu})[/color]",
                f"{rc_p} [color=D32F2F]({rc_pu})[/color]", f"{rc_w} [color=D32F2F]({rc_wu})[/color]",
                f"{tot_p} [color=D32F2F]({tot_pu})[/color]", f"{tot_w} [color=D32F2F]({tot_wu})[/color]"
            ))
...
class UnifiedTaskCard(RecycleDataViewBehavior, BoxLayout):
    ...
    def refresh_view_attrs(self, rv, index, data):
        ...
        qty_per_box = safe_int(t(self.task_data, '박스입수량', t(self.task_data, '박스 입수량', 0)))
        ...
        if is_shelf_rack: tag_prefix += "[color=1565C0][선반랙][/color] "
        if is_invoice_only: tag_prefix += "[color=D32F2F](송장만부착)[/color] "
        ...
        self.ids.lbl_loc.text = f"[color=D32F2F]{from_loc}[/color] ➔ [color=1E88E5]{to_loc}[/color]"

        req_qty = str(t(self.task_data, '지시수량', '0'))
        total_qty = safe_int(req_qty)
        box_ea_str = f"({total_qty // qty_per_box}B/{total_qty % qty_per_box}E)" if qty_per_box > 0 else f"({total_qty}E)"
...
class UnifiedReplenishScreen(Screen):
    def __init__(self, **kwargs):
        ...
        btn_back = StyledButton(text='< 뒤로', size_hint_x=0.2, bg_color=get_color_from_hex('#78909C'))
        btn_back.bind(on_press=lambda x: setattr(self.manager, 'current', 'main_menu'))
        
        lbl_title = Label(text='보충 작업 통합 컨트롤', font_name=FONT_NAME, font_size=dp(16), bold=True, color=TEXT_DARK)
        
        btn_refresh = StyledButton(text='갱신', size_hint_x=0.2)
        btn_refresh.bind(on_press=lambda x: self.fetch_data())
        
        header.add_widget(btn_back); header.add_widget(lbl_title); header.add_widget(btn_refresh)
        self.layout.add_widget(header)

        # 2. 필터 토글 버튼
        self.btn_toggle_filter = StyledButton(text='🔍 필터 설정 닫기 ▲ (대기작업 / 전체)', size_hint_y=None, height=dp(32), bg_color=get_color_from_hex('#546E7A'), font_size=dp(12))
        self.btn_toggle_filter.bind(on_press=self.toggle_filter_panel)
        self.layout.add_widget(self.btn_toggle_filter)

        # 3. 📦 필터 종합 컨테이너 패널 구성
        self.filter_panel = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(112), spacing=dp(4))
        
        # 3-1. 탭 선택
        main_tab_box = BoxLayout(size_hint_y=None, height=dp(36), spacing=dp(5))
        self.btn_tab_pending = StyledToggleButton(text='대기 작업', group='main_tab', state='down', size_hint_x=0.5, font_size=dp(14))
        self.btn_tab_pending.bind(on_press=lambda x: self.switch_main_tab('PENDING'))
        
        self.btn_tab_my = StyledToggleButton(text='내 작업', group='main_tab', state='normal', size_hint_x=0.5, font_size=dp(14))
        self.btn_tab_my.bind(on_press=lambda x: self.switch_main_tab('MY'))
        
        main_tab_box.add_widget(self.btn_tab_pending); main_tab_box.add_widget(self.btn_tab_my)
        self.filter_panel.add_widget(main_tab_box)

        # 3-2. 장비 선택
        equip_filter_box = BoxLayout(size_hint_y=None, height=dp(32), spacing=dp(5))
        self.btn_eq_all = StyledToggleButton(text='전체', group='equip_filter', state='down', font_size=dp(12))
        self.btn_eq_all.bind(on_press=lambda x: self.switch_equip_filter('ALL'))
        
        self.btn_eq_op = StyledToggleButton(text='오더피커', group='equip_filter', state='normal', font_size=dp(12))
        self.btn_eq_op.bind(on_press=lambda x: self.switch_equip_filter('ORDERPICKER'))
        
        self.btn_eq_reach = StyledToggleButton(text='리치', group='equip_filter', state='normal', font_size=dp(12))
        self.btn_eq_reach.bind(on_press=lambda x: self.switch_equip_filter('REACH'))
        
        equip_filter_box.add_widget(self.btn_eq_all); equip_filter_box.add_widget(self.btn_eq_op); equip_filter_box.add_widget(self.btn_eq_reach)
        self.filter_panel.add_widget(equip_filter_box)

        # 3-3. 로케이션 & 긴급 필터
        opt_toolbar = BoxLayout(size_hint_y=None, height=dp(32), spacing=dp(5))
        from_zone_list = ["보관: 전체", "I존", "J존", "K존", "L존", "M존", "N존", "O존", "P존", "Q존", "R존"]
        self.sp_from = StyledSpinner(text="보관: 전체", values=from_zone_list, font_size=dp(12), size_hint_x=0.36, option_cls=KoreanSpinnerOption)
        self.sp_from.bind(text=self.on_from_spinner_change)
        
        self.sp_to = StyledSpinner(text="이동: 전체", values=["이동: 전체", "A존", "B존", "C존", "D존", "I존", "O존"], font_size=dp(12), size_hint_x=0.36, option_cls=KoreanSpinnerOption)
        self.sp_to.bind(text=self.on_to_spinner_change)

        chk_box = BoxLayout(size_hint_x=0.28, spacing=dp(2))
        self.chk_urgent = CheckBox(active=False, size_hint_x=None, width=dp(22), color=PRIMARY_BLUE)
        self.chk_urgent.bind(active=self.on_urgent_check_change)
        lbl_urg = Label(text='긴급만', font_name=FONT_NAME, font_size=dp(12), color=TEXT_DARK, halign='left', valign='middle')
        lbl_urg.bind(size=lambda i, s: setattr(i, 'text_size', s))
        
        chk_box.add_widget(self.chk_urgent); chk_box.add_widget(lbl_urg)
        opt_toolbar.add_widget(self.sp_from); opt_toolbar.add_widget(self.sp_to); opt_toolbar.add_widget(chk_box)
        ...
        self.lbl_status_count = Label(text='대기 작업 (전체) : 0건', font_name=FONT_NAME, font_size=dp(13), color=TEXT_MUTED, halign='left')
        ...
        self.lbl_chk_all = Label(text='전체선택', font_name=FONT_NAME, font_size=dp(12), color=TEXT_DARK, size_hint_x=None, width=dp(55))
        
        list_header.add_widget(self.lbl_status_count); list_header.add_widget(self.chk_all); list_header.add_widget(self.lbl_chk_all)
        self.layout.add_widget(list_header)

        # 5. 메인 리스트뷰
        self.rv = RecycleView()
        self.rv_layout = RecycleBoxLayout(default_size=(None, dp(170)), default_size_hint=(1, None), size_hint_y=None, orientation='vertical', spacing=dp(8))
        ...
        self.btn_claim_action = StyledButton(text='+ 선택 항목 할당받기 (0)', size_hint_x=None, width=dp(210), bg_color=PRIMARY_BLUE, bold=True, font_size=dp(13))
    ...
    def _update_filter_button_text(self, arrow):
        ...
        eq_str = "전체" if self.active_equip_filter == 'ALL' else ("오더피커" if self.active_equip_filter == 'ORDERPICKER' else "리치")
        self.btn_toggle_filter.text = f"🔍 필터 설정 {arrow} ({tab_str} / {eq_str})"
    ...
    def on_enter(self):
        ...
        self.action_bar.height = dp(42); self.action_bar.opacity = 1; self.action_bar.disabled = False
        self.chk_all.opacity = 1; self.lbl_chk_all.opacity = 1; self.chk_all.disabled = False
        self.fetch_data()
    ...
    def handle_barcode_scan(self, barcode):
        ...
        my_matches = [
            t_item for t_item in self.raw_all_tasks
            if str(t(t_item, '상태')).strip() == '작업중'
            and str(t(t_item, '작업 담당자')).strip().lower() == user_name
            and str(t(t_item, '상품바코드', t(t_item, '바코드', ''))).strip().upper() == clean_bc
        ]
        ...
        if my_matches:
            ...
            if pending_matches:
                ...
                app.show_info_popup("스캔 알림", f"['내 작업' 목록에 없는 상품입니다]\n대기 목록의 [{clean_bc}] 항목이 체크 선택되었습니다.\n하단의 [할당받기]를 먼저 눌러주세요.")
            else:
                app.show_info_popup("스캔 오류", f"스캔한 바코드 [{clean_bc}] 에 해당하는 작업을 찾을 수 없습니다.")
    ...
    def switch_main_tab(self, tab_mode):
        ...
        if tab_mode == 'MY':
            self.action_bar.height = 0; self.action_bar.opacity = 0; self.action_bar.disabled = True
            self.chk_all.opacity = 0; self.lbl_chk_all.opacity = 0; self.chk_all.disabled = True
        else:
            self.action_bar.height = dp(42); self.action_bar.opacity = 1; self.action_bar.disabled = False
            self.chk_all.opacity = 1; self.lbl_chk_all.opacity = 1; self.chk_all.disabled = False

        self._update_filter_button_text("닫기 ▲" if self.is_filter_expanded else "열기 ▼")
        self.apply_filters_and_render()
    ...
    def on_from_spinner_change(self, spinner, text):
        self.selected_from_zone = "전체" if "전체" in text else text.replace("존", "").strip()
        self.apply_filters_and_render()

    def on_to_spinner_change(self, spinner, text):
        self.selected_to_zone = "전체" if "전체" in text else text.replace("존", "").strip()
        self.apply_filters_and_render()
    ...
    def toggle_card_check(self, task_data, is_checked):
        ...
        if is_checked: self.checked_task_ids.add(task_id)
        else: self.checked_task_ids.discard(task_id)
        self.btn_claim_action.text = f"+ 선택 항목 할당받기 ({len(self.checked_task_ids)})"

    def on_check_all_change(self, checkbox, value):
        if self.active_main_tab == 'MY': return
        self.rv.data = [{**item, 'is_checked': value} for item in self.rv.data]
        self.rv.refresh_from_data()
        if value:
            for item in self.rv.data: self.checked_task_ids.add(t(item['task_data'], '작업ID'))
        else:
    ...
    def apply_filters_and_render(self):
        ...
        for task in self.raw_all_tasks:
            ...
            if self.active_main_tab == 'PENDING':
                if status != '대기' or assignee != '': continue
            else:
                if status != '작업중' or assignee != user_name: continue

            equip = str(t(task, '장비')).strip()
            is_urg = t(task, '긴급여부') == 'Y'

            eq_all_tot += 1
            if is_urg: eq_all_urg += 1

            if equip == '오더피커':
                eq_op_tot += 1
                if is_urg: eq_op_urg += 1
            elif equip == '리치':
                eq_reach_tot += 1
                if is_urg: eq_reach_urg += 1
        ...
        # 💡 2. 상단 장비 메뉴바 버튼 텍스트 업데이트 (총수량 / 긴급건수)
        ...
        for task in self.raw_all_tasks:
            ...
            if self.active_equip_filter == 'ORDERPICKER' and equip != '오더피커': continue
            if self.active_equip_filter == 'REACH' and equip != '리치': continue

            from_loc = str(t(task, '기존로케이션')).strip().upper()
            to_loc = str(t(task, '보충로케이션')).strip().upper()

            clean_from_sel = self.selected_from_zone.split('존')[0].replace("보관:", "").strip()
            clean_to_sel = self.selected_to_zone.split('존')[0].replace("이동:", "").strip()

            if '전체' not in clean_from_sel and not from_loc.startswith(clean_from_sel): continue
            if '전체' not in clean_to_sel and not to_loc.startswith(clean_to_sel): continue

            if self.only_urgent and t(task, '긴급여부') != 'Y': continue

            filtered_list.append(task)

        filtered_list.sort(key=lambda x: (t(x, '긴급여부') != 'Y', t(x, '기존로케이션', '')))
        ...
        eq_name = "전체" if self.active_equip_filter == 'ALL' else ("오더피커" if self.active_equip_filter == 'ORDERPICKER' else "리치")
    ...
    def claim_checked_tasks(self, instance):
        if not self.checked_task_ids:
            App.get_running_app().show_info_popup("알림", "할당받을 작업을 선택해주세요.")
            return
        App.get_running_app().show_loading_popup()
    ...
    def on_claim_success(self):
        App.get_running_app().show_info_popup("성공", "선택한 작업이 '내 작업'으로 할당되었습니다.")
    ...
    def handle_my_task_action(self, action_name, task_data):
        ...
        if action_name == 'return': task_list_screen.return_task(dummy_card)
        elif action_name == 'qty': task_list_screen.open_quantity_popup(dummy_card)
        elif action_name == 'fail': task_list_screen.process_failure(dummy_card)
        elif action_name == 'remarks': task_list_screen.prompt_for_remarks(dummy_card)
        elif action_name == 'complete': task_list_screen.process_task(dummy_card)
...
class TaskListScreen(Screen):
    ...
    def refresh_list(self, force_refresh=True):
        App.get_running_app().show_loading_popup()
        threading.Thread(target=self._perform_get_tasks, args=(force_refresh,), daemon=True).start()

    def _perform_get_tasks(self, force_refresh):
        try:
            all_tasks = get_sheet_data(TASK_SHEET_NAME, force_refresh=force_refresh)
            filtered_task_data = [task for task in all_tasks if str(t(task, '상태')).strip() == '보충완료']
            
            task_map = {t(task, '작업ID'): (i + 2, task) for i, task in enumerate(all_tasks)}
            my_tasks = [{'task_data': task, 'task_screen': self, 'row_index': task_map.get(t(task, '작업ID'), (0,None))[0]} for task in filtered_task_data if t(task, '작업ID') in task_map]
    ...
    def update_recycle_view(self):
        ...
        for item in self.all_tasks_data:
            ...
            barcode = str(t(task, '상품바코드', t(task, '바코드', ''))).lower()
    ...
    def handle_barcode_scan(self, barcode):
        matches = [item for item in self.all_tasks_data if str(t(item['task_data'], '상품바코드', t(item['task_data'], '바코드', ''))).strip().upper() == barcode]
        if not matches:
            App.get_running_app().show_info_popup("알림", f"스캔한 바코드[{barcode}]는 검수 대상 목록에 없습니다.")
            return
        if len(matches) == 1: self.open_task_from_scan(matches[0])
        else: MultipleSkuSelectPopup(matches=matches, on_select=self.open_task_from_scan).open()

    def open_task_from_scan(self, item_data):
        dummy_card = type('DummyCard', (), {'task_data': item_data['task_data']})()
    ...
    def return_task(self, card):
        App.get_running_app().show_confirmation_popup("작업 반납", "이 작업을 반납하시겠습니까?", lambda: self._perform_return(card))

    def _perform_return(self, card):
        App.get_running_app().show_loading_popup()
        threading.Thread(target=self._async_return, args=(card,), daemon=True).start()
    ...
    def _async_return(self, card):
        try:
            ...
            row_idx = sheet.col_values(headers.index('작업ID') + 1).index(task_id) + 1
            sheet.update_cells([gspread.Cell(row_idx, headers.index('상태') + 1, '대기'), gspread.Cell(row_idx, headers.index('작업 담당자') + 1, '')])
            invalidate_cache(TASK_SHEET_NAME)
            Clock.schedule_once(lambda dt: self.on_action_success("작업이 반납되었습니다."))
        except Exception as e: Clock.schedule_once(lambda dt, err=str(e): App.get_running_app().show_info_popup("오류", str(err)))
        finally: Clock.schedule_once(lambda dt: App.get_running_app().dismiss_loading_popup())

    def open_quantity_popup(self, card):
        ordered_qty = str(t(card.task_data, '지시수량', '0')).strip()
        existing_conf_qty = str(card.task_data.get('confirmed_quantity', '')).split('/')[0].strip()
        initial_val = existing_conf_qty if existing_conf_qty.isdigit() else ordered_qty
        
        def on_confirm_qty(text):
            if text.strip().isdigit():
                card.task_data['confirmed_quantity'] = text.strip()
                App.get_running_app().show_info_popup("알림", f"수량 '{text}'(이)가 임시 저장되었습니다.")
            else: App.get_running_app().show_info_popup("오류", "숫자만 입력해야 합니다.")

        SingleInputPopup(title="확인 수량 입력", hint_text="수량 입력", on_confirm=on_confirm_qty, initial_text=initial_val, input_type='number').open()
    ...
    def prompt_for_remarks(self, card):
        def on_confirm_rem(text):
            ...
            curr = card.task_data.get('remarks_text', t(card.task_data, '비고', ''))
            card.task_data['remarks_text'] = f"{curr}\n{formatted}" if curr else formatted
            app.show_info_popup("알림", "비고가 추가되었습니다.")

        SingleInputPopup(title="비고 추가", hint_text="비고 내용 입력", on_confirm=on_confirm_rem).open()

    def process_failure(self, card):
        def on_confirm_fail(reason):
            if not reason.strip(): return
            app = App.get_running_app()
            updates = {'상태': '보충실패', '실패사유': reason.strip(), '보충담당자': app.user_real_name, '완료일시': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
            app.show_loading_popup()
            threading.Thread(target=self._perform_update, args=(card, updates, "보충 실패 처리 되었습니다."), daemon=True).start()

        SingleInputPopup(title="실패 사유 입력", hint_text="사유 입력", on_confirm=on_confirm_fail).open()
    ...
    def process_task(self, card):
        ...
        if app.current_list_type == '검수인원':
            ...
            return
        qty_str = str(card.task_data.get('confirmed_quantity', '')).split('/')[0].strip()
        if not qty_str.isdigit() or int(qty_str) == 0:
            app.show_info_popup('입력 오류', '수량입력 버튼을 눌러 확인 수량을 먼저 입력해주세요.')
            return
        self._finalize_task_processing(card, int(qty_str), 0, None, card.task_data.get('remarks_text', t(card.task_data, '비고', '')))

    def _start_print_job(self, card_data):
        try:
            app = App.get_running_app()
            default_printer = app.load_default_printer()
            if not default_printer: return
            qty_per_box = safe_int(t(card_data, '박스입수량', 0))
            prod_name = str(t(card_data, '상품명', ''))
            is_invoice_only = (qty_per_box == 1 or '송장' in prod_name)
            label_info = {'바코드': str(t(card_data, '상품바코드', 'N/A')), '출고 로케이션': str(t(card_data, '보충로케이션', 'N/A')), '긴급여부': t(card_data, '긴급여부') == 'Y', '송장전용': is_invoice_only}
            threading.Thread(target=self._print_thread, args=(default_printer, label_info, 1), daemon=True).start()
        except Exception as e:
    ...
    def _print_thread(self, address, label_info, quantity):
        try:
            ...
            if printer.connect():
                try: printer.print_outbound_label_cpcl(label_info, quantity)
                except Exception: pass
                finally: printer.disconnect()
        except Exception as e:
            print(f"🔴 블루투스 인쇄 실패: {e}")

    def _finalize_task_processing(self, card, final_qty, split_qty, final_location, updated_remarks):
        app = App.get_running_app()
        updates = {'상태': '보충완료', '보충담당자': app.user_real_name, '완료일시': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '확인수량': final_qty, '비고': updated_remarks}
        
        def run_sheet_update():
            app.show_loading_popup()
            threading.Thread(target=self._perform_update, args=(card, updates, "보충완료 처리 되었습니다."), daemon=True).start()

        def on_yes_print():
            try: self._start_print_job(card.task_data)
            except Exception: pass
            run_sheet_update()

        app.show_confirmation_popup(title="인쇄 및 완료", message="[color=ffffff]보충완료 처리합니다.\n라벨 1장을 인쇄하시겠습니까?[/color]", on_yes=on_yes_print, on_no=run_sheet_update)
    ...
    def _perform_update(self, card, updates, msg):
        try:
            ...
            task_id = str(t(card.task_data, '작업ID'))
            row_idx = sheet.col_values(headers.index('작업ID') + 1).index(task_id) + 1
            cells = [gspread.Cell(row_idx, headers.index(k) + 1, str(v)) for k, v in updates.items() if k in headers]
...
class AdminDashboardScreen(Screen):
    def __init__(self, **kwargs):
        ...
        top_bar.add_widget(StyledButton(text='< 메인', size_hint_x=0.2, bg_color=get_color_from_hex('#78909C'), on_press=lambda x: setattr(self.manager, 'current', 'main_menu')))
        top_bar.add_widget(Label(text='전체 작업 현황판', font_name=FONT_NAME, font_size=dp(18), bold=True, color=TEXT_DARK))
        top_bar.add_widget(StyledButton(text='갱신', size_hint_x=0.2, on_press=lambda x: self.refresh()))
        self.layout.add_widget(top_bar)
        ...
        btn_search = StyledButton(text='검색', size_hint_x=0.2, font_size=dp(14))
    ...
    def on_enter(self):
        ...
        self.grid.add_widget(Label(text='바코드를 스캔하거나 입력 후 검색해 주세요.', font_name=FONT_NAME, font_size=dp(15), color=TEXT_MUTED, size_hint_y=None, height=dp(100)))
    ...
    def search_tasks(self, instance=None):
        ...
        if not matches:
            self.grid.add_widget(Label(text=f"검색어 [{query}] 에 해당하는 작업 건이 없습니다.", font_name=FONT_NAME, font_size=dp(14), color=TEXT_MUTED, size_hint_y=None, height=dp(100)))
            return
    ...
    def _create_dashboard_card(self, task):
        ...
        card = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(110), padding=dp(10), spacing=dp(4))
        with card.canvas.before:
            Color(1, 1, 1, 1)
            RoundedRectangle(pos=card.pos, size=card.size, radius=[dp(10),])
        card.bind(pos=lambda i,p: setattr(i.canvas.before.children[-1], 'pos', p), size=lambda i,s: setattr(i.canvas.before.children[-1], 'size', s))

        top_row = BoxLayout(size_hint_y=None, height=dp(25))
        st_color = "[color=3366ff]" if status == '작업중' else ("[color=00897B]" if status in ['보충완료', '최종완료'] else "[color=757575]")
        urg_tag = "[color=D32F2F][긴급][/color] " if is_urgent else ""
        
        lbl_prod = Label(text=f"{st_color}[b][{status}][/b][/color] {urg_tag}{t(task, '상품명')}", font_name=FONT_NAME, font_size=dp(14), markup=True, halign='left')
        lbl_prod.bind(size=lambda i,s: setattr(i, 'text_size', s))
        top_row.add_widget(lbl_prod)
        card.add_widget(top_row)

        lbl_info = Label(text=f"바코드: {t(task, '상품바코드', t(task, '바코드'))} | {t(task, '기존로케이션')} ➔ {t(task, '보충로케이션')}", font_name=FONT_NAME, font_size=dp(13), color=TEXT_DARK, halign='left')
        ...
        assignee = t(task, '검수담당자', t(task, '보충담당자', t(task, '작업 담당자', '미할당')))
        lbl_sub = Label(text=f"담당: {assignee} / 수량: {t(task, '확인수량', t(task, '지시수량', 0))}EA{time_str}", font_name=FONT_NAME, font_size=dp(12), color=TEXT_MUTED, markup=True, halign='left')
...
class CompletedHistoryScreen(Screen):
    def __init__(self, **kwargs):
        ...
        top.add_widget(StyledButton(text='< 메인', size_hint_x=0.2, bg_color=get_color_from_hex('#78909C'), on_press=lambda x: setattr(self.manager, 'current', 'main_menu')))
        top.add_widget(Label(text='금일 완료 이력', font_name=FONT_NAME, font_size=dp(18), bold=True, color=TEXT_DARK))
...
class SettingsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        header.add_widget(StyledButton(text='< 메인', size_hint_x=0.2, bg_color=get_color_from_hex('#78909C'), on_press=lambda x: setattr(self.manager, 'current', 'main_menu')))
        header.add_widget(Label(text='프린터 설정', font_name=FONT_NAME, font_size=dp(18), bold=True, color=TEXT_DARK))
        layout.add_widget(header)
        layout.add_widget(StyledButton(text='프린터 연결 설정', size_hint_y=None, height=dp(55), on_press=lambda x: Factory.PrinterSettingsPopup().open()))
        self.add_widget(layout)

class PrinterSettingsPopup(Popup):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.title = "프린터 설정"; self.title_font = FONT_NAME; self.size_hint = (0.9, None); self.height = dp(350); self.auto_dismiss = False
        self.found_devices = {}
        layout = BoxLayout(orientation='vertical', padding=dp(10), spacing=dp(10))
        self.current_lbl = Label(text="현재 프린터: " + (App.get_running_app().load_default_printer() or "미설정"), font_name=FONT_NAME)
        layout.add_widget(self.current_lbl)
        self.spinner = StyledSpinner(text="기기를 선택하세요", font_name=FONT_NAME, option_cls=KoreanSpinnerOption, size_hint_y=None, height=dp(45))
        layout.add_widget(self.spinner)
        
        btn_scan = StyledButton(text="주변 기기 검색", on_press=self.scan_devices, size_hint_y=None, height=dp(45))
        btn_save = StyledButton(text="기본 프린터로 저장", on_press=self.save_printer, size_hint_y=None, height=dp(45))
        btn_close = StyledButton(text="닫기", on_press=self.dismiss, size_hint_y=None, height=dp(45), bg_color=(0.6,0.6,0.6,1))
        layout.add_widget(btn_scan); layout.add_widget(btn_save); layout.add_widget(btn_close)
        self.content = layout

    def scan_devices(self, instance):
        if platform != 'android' or not autoclass: return
        try:
            adapter = autoclass('android.bluetooth.BluetoothAdapter').getDefaultAdapter()
            if not adapter.isEnabled(): return
            self.found_devices.clear()
            for dev in adapter.getBondedDevices().toArray(): self.found_devices[dev.getName()] = dev.getAddress()
            self.spinner.values = list(self.found_devices.keys()) if self.found_devices else ["페어링된 기기 없음"]
        except Exception: pass

    def save_printer(self, instance):
        name = self.spinner.text
        if name in self.found_devices:
            App.get_running_app().save_default_printer(self.found_devices[name])
            self.current_lbl.text = "현재 프린터: " + self.found_devices[name]
            self.dismiss()

class BluetoothPrinter:
    def __init__(self, device_address):
        self.mac_address = device_address; self.socket = None; self.stream = None
    ...
    def connect(self):
        ...
        try:
            ...
            device = BluetoothAdapter.getDefaultAdapter().getRemoteDevice(self.mac_address)
            spp_uuid = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB")
            self.socket = device.createRfcommSocketToServiceRecord(spp_uuid)
            self.socket.connect(); self.stream = self.socket.getOutputStream()
            return True
...
class MainApp(App):
    ...
    def on_start(self):
        ...
        if platform == 'android':
            try:
                request_permissions([
                    Permission.POST_NOTIFICATIONS, Permission.BLUETOOTH_SCAN,
                    Permission.BLUETOOTH_CONNECT, Permission.BLUETOOTH_ADMIN, Permission.ACCESS_FINE_LOCATION 
                ])
            except Exception as e: print(f"🔴 권한 요청 오류: {e}")
    ...
    def save_user_name(self, name):
        try:
            with open(self.get_config_path(), 'w') as f: json.dump({'user_name': name}, f)
        except Exception: pass
    ...
    def load_saved_user_name(self):
        ...
        if os.path.exists(path):
            try:
                with open(path, 'r') as f: return json.load(f).get('user_name')
            except Exception: None
        return None

    def save_default_printer(self, address):
        try:
            with open('printer_config.json', 'w') as f: json.dump({'default_printer': address}, f)
        except Exception: pass

    def load_default_printer(self):
        if os.path.exists('printer_config.json'):
            try:
                with open('printer_config.json', 'r') as f: return json.load(f).get('default_printer')
            except Exception: None
        return None

    def show_loading_popup(self):
        if not self.loading_popup.parent: self.loading_popup.open()
    def dismiss_loading_popup(self): self.loading_popup.dismiss()
    def show_info_popup(self, title, message): InfoPopup(title, message).open()
    def show_confirmation_popup(self, title, message, on_yes, on_no=None):
        ...
        msg_label = Label(text=str(message), halign='center', valign='middle', markup=True, color=(1,1,1,1), size_hint_y=1, font_size=dp(18), font_name=FONT_NAME)
        msg_label.bind(size=msg_label.setter('text_size'))
        button_layout = BoxLayout(size_hint_y=None, height=dp(45), spacing=dp(10))
        btn_yes = StyledButton(text='예'); btn_no = StyledButton(text='아니오')
        button_layout.add_widget(btn_yes); button_layout.add_widget(btn_no)
        popup_content.add_widget(msg_label); popup_content.add_widget(button_layout)
        popup = Popup(title=title, title_font=FONT_NAME, content=popup_content, size_hint=(0.8, None), height=dp(280), auto_dismiss=False, background_color=(0.3, 0.3, 0.3, 0.95))
        
        def yes_action(instance):
            popup.dismiss()
            if on_yes: Clock.schedule_once(lambda dt: on_yes(), 0.1)
            
        def no_action(instance):
            popup.dismiss()
            if on_no: Clock.schedule_once(lambda dt: on_no(), 0.1)
